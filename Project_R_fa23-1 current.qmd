...
```{r}
bbnames[, .(total = sum(counts)), by = .(sex, name)][order(sex, -total)][,  .SD[1], by = sex]
```
...
```{r}
bbnames_alive <- bbnames[, age := 2026 - year]
bbnames_alive <- bbnames[age <= 85]
bbnames_alive
```
...
ggplot(john_data, aes(x = year, y = counts)) +
  ...
  labs(
    title = "Distribution of the name 'John' over time",
    x = "Year",
    y = "Counts"
  )
...
```{r}
mary_data <- bbnames[name == "Mary"]

library(ggplot2)

ggplot(mary_data, aes(x = year, y = counts)) +
  geom_bar(stat = "identity", alpha = 0.3) +
  geom_line() +
  facet_wrap(~sex, scales = "free") +
  labs(
    title = "Distribution of the name 'Mary' over time",
    x = "Year",
    y = "Counts"
  )
